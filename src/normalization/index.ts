...
export function normalizeSignals(rawSignals: Signals): Signals {
  ...
  if (rawSignals.momentum.available && rawSignals.momentum.value !== null) {
    ...
    normalized.momentum.value = normalizeValue(
      absChange,
  ...
  if (rawSignals.liquidity.available && rawSignals.liquidity.value !== null) {
    normalized.liquidity.value = normalizeValue(
      rawSignals.liquidity.value,
  ...
  // Normalize holder change (percentage change)
  if (rawSignals.holders.available && rawSignals.holders.value !== null) {
    // We need previous holder count to calculate change
    // For now, if we only have current count, we can't calculate change
    // This would require historical data
    normalized.holders.value = null;
    normalized.holders.available = false;
    normalized.holders.metadata = {
      ...rawSignals.holders.metadata,
      error: 'Holder change calculation requires historical data'
    };
  }
